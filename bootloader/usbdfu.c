...
static const char *usb_strings[] = {
	...
	"@Internal Flash   /0x08000000/8*001Ka,120*001Kg",		// correct for a 128kb stm32?
};
...
// check for 0xb007 in bkp_dr1
static int forcedfumagicval(void)
{
	uint32_t magicvalue;
	...
	return ((magicvalue==MAGICVALUE)?1:0);
}

// check if swdio and swdclk are shorted
static int forcedfuuser(void)
{
	int count=0;

	// only swd pins are exposed
	// activate the bootloader if swdio and swdclk are shorted (oldskoolbuspirate) 
	...
	// setup IO
	// A13 is output, A14 is input
	gpio_set_mode(GPIOA, GPIO_MODE_OUTPUT_2_MHZ, GPIO_CNF_OUTPUT_PUSHPULL, GPIO13);
	gpio_set_mode(GPIOA, GPIO_MODE_INPUT, GPIO_CNF_INPUT_FLOAT,GPIO14);

	// a13 -> 1
	gpio_set(GPIOA, GPIO13);
	if(gpio_get(GPIOA, GPIO14)) count++;
	// a13 -> 0
	gpio_clear(GPIOA, GPIO13);
	if(!gpio_get(GPIOA, GPIO14)) count++;

	// enable swd/jtag
	AFIO_MAPR = 0x00000000;				// this is the powerup value.
...
int main(void)
{
	...
	if ((!forcedfuuser())&&(!forcedfumagicval()))
	{
...
//	rcc_clock_setup_in_hsi_out_48mhz();
	...
	gpio_set_mode(GPIOE, GPIO_MODE_OUTPUT_2_MHZ, GPIO_CNF_OUTPUT_PUSHPULL, GPIO6);
